...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler({Exception    .class, RuntimeException.class, Throwable.class, BadCredentialsException.class}) // if these exceptions happen
    public ResponseEntity<ResponseWrapper> genericException(Throwable e, HandlerMethod handlerMethod) {

        Optional<DefaultExceptionMessageDto> defaultMessage = getMessageFromAnnotation(handlerMethod.getMethod());

        if (defaultMessage.isPresent() && !ObjectUtils.isEmpty(defaultMessage.get().getMessage())) { // AND my custom annotation @DefaultExceptionMessage is on top of that method
            ResponseWrapper response = ResponseWrapper // do these
                    .builder()
                    .success(false)
        ...
        return new ResponseEntity<>(ResponseWrapper.builder().success(false).message("Action failed: An error occurred!").code(HttpStatus.INTERNAL_SERVER_ERROR.value()).build(), HttpStatus.INTERNAL_SERVER_ERROR); // BUT my exception annotation is not there, do this
    }
