...
class ProjectControllerTest {
    ...
    private MockMvc mvc;
    static String token;
    static UserDTO manager; // we will be using these in the @BeforeAll etc which are static, and static accepts static
    static ProjectDTO project;

    @BeforeAll // creating some sample object/data
    static void setUp() {
    ...
    void givenNoToken_getProjects() throws Exception {
        ...
        mvc.perform(MockMvcRequestBuilders.get("/api/v1/project")) // since we did not provide any token, we expect an exception, if there is, this method will pass
                .andExpect(status().is4xxClientError()); // it is checking for all 4xx codes
    ...
    void givenToken_getProjects() throws Exception {
        ...
        mvc.perform(MockMvcRequestBuilders.get("/api/v1/project")
                ...
                .andExpect(jsonPath("$.data[0].projectCode").exists()) // give me the first element inside data-projects
                .andExpect(jsonPath("$.data[0].assignedManager.userName").exists()) // we have the field or not
    ...
    void givenToken_createProject() throws Exception {
        ...
        mvc.perform(MockMvcRequestBuilders.post("api/v1/project")
                .header("Authorization", token)
                .contentType(MediaType.APPLICATION_JSON) // the type of the request body i send
                .accept(MediaType.APPLICATION_JSON) // the type i want to receive in response body
